...
### 11주차 - 적외선(IR) 통신
- **IR_Transmit_NEC**: 적외선 송신기로 NEC 프로토콜 데이터 전송
- **IR_Receive_Basic**: 적외선 수신기로 데이터 수신 및 출력
- **IR_Remote_LED_Control**: 적외선 리모컨으로 LED 제어
...
- **보드**: 
  - **1-7주차**: Arduino Uno 또는 호환 보드
  - **8-9주차**: DM-MSTYLE-EXT 보드 (Arduino Mega 기반)

## � 필수 라이브러리 설치

### IRremote 라이브러리 (11주차 적외선 통신)

> [!IMPORTANT]
> **IRremote 라이브러리 버전 주의사항**  
> 11주차 적외선 통신 실습을 위해서는 반드시 **IRremote 라이브러리 버전 2.6.1**을 설치해야 합니다. 최신 버전(3.x 이상)은 API가 변경되어 예제 코드와 호환되지 않습니다.

**설치 방법**:
1. Arduino IDE에서 `스케치 > 라이브러리 포함하기 > 라이브러리 관리` 선택
2. 검색창에 "IRremote" 입력
3. **반드시 버전 2.6.1 선택** (드롭다운 메뉴에서 버전 선택 가능)
4. "설치" 버튼 클릭

## �📋 사용 방법

### 1. Arduino IDE 설치
[Arduino 공식 웹사이트](https://www.arduino.cc/en/software)에서 Arduino IDE를 다운로드하여 설치
...
- **11주차**: 적외선 송신기 (IR LED), 적외선 수신기, 적외선 리모컨

> [!IMPORTANT]
> **DM-MSTYLE-EXT 보드 안내**  
> 8주차와 9주차 실습에서는 DM-MSTYLE-EXT 보드를 사용합니다. 이 보드는 Arduino Mega 기반으로 제작, 7 Segment 디스플레이와 LCD가 이미 내장되어 있어 별도의 배선 없이 실습을 진행 가능
> - 7 Segment: 핀 58-69번 사용
> - LCD 1602: 핀 44-49번 사용

> [!NOTE]
> 각 실습 코드의 주석을 참고하여 핀 연결을 확인 필요, 대부분의 코드에는 어느 핀에 부품을 연결해야 하는지 명시되어 있음
...
- **기존**: Arduino Uno
- **변경**: **DM-MSTYLE-EXT** (ATmega2560 호환 보드)

### 2. 주요 이슈 (Issue Log)

- **발견 경위**: 실습 조교 자료 제작을 위해 샘플 20개 반출 및 테스트 중 발견
- **문제점 진단**
    - 과거 AVR ISP 방식으로 사용된 것으로 추정됨 (현재 **부트로더가 삭제된 상태**)
    - 아두이노 IDE에서 바로 업로드 불가, 부트로더 재설치 필요
- **장애 요인**
    - 과거 자료에 있는 '프로그래머 파일'이 현재 소실됨
    - 제조사 제공 불가 확인 (자체 생성 또는 제작 필요)

### 3. 조치 계획 (Action Plan)

- [ ] **부트로더 복구**: 호환 가능한 프로그래머/부트로더 파일 자체 생성 및 확보
- [x] **키트 전수 조사 (Full Inspection)**
    
    > **사유**: 전년도와 불량 유형 및 정상 작동 기준이 상이함. 샘플(20개) 외 전체 수량에 대한 상태 확인 필수.
    
    - [x] 전체 키트 부트로더 유무 확인
    - [x] 불량/정상 분류 및 라벨링

> [!WARNING]
> **DM-MSTYLE-EXT 보드 사용 시 주의사항**  
> 부트로더가 없는 보드는 Arduino IDE에서 직접 업로드가 불가능 => 반드시 사전에 부트로더 상태를 확인하고, 필요 시 부트로더를 재설치한 후 사용 권장

## �📝 참고사항

- 모든 코드는 교육 목적으로 작성되었으며, 학생들의 학습을 돕기 위해 주석이 포함되어 있음
- 코드는 단계별로 난이도가 증가하도록 설계됨
- 실습 중 문제가 발생하면 시리얼 모니터를 통해 디버깅하는 것을 권장

## 🤝 기여

본 저장소는 IoT 기초실습 조교 활동의 일환으로 제작됨.
코드 개선이나 추가 실습 예제에 대한 제안은 언제든 환영

## 📄 라이센스

본 실습 코드는 교육 목적으로 자유롭게 사용 가능
